...
test('can create, join, start, and advance a turn', async ({ page }) => {
	...
	await page.waitForResponse((response) => response.url().includes('/turn?'));
	...
	expect(requests.some((url) => url.includes('/alignment?'))).toBeTruthy();
	expect(requests.some((url) => url.includes('/completeturn?'))).toBeTruthy();
	expect(requests.some((url) => url.includes('/turn_finale?'))).toBeTruthy();
	expect(requests.some((url) => url.includes('/process/turn?'))).toBeTruthy();
});

test('join_game encodes prompts safely', async ({ page }) => {
	...
	const joinRequestPromise = page.waitForRequest((request) =>
		request.url().includes('/join_game?')
	);
	await page.getByRole('button', { name: 'Join' }).click();

	const joinRequest = await joinRequestPromise;
	const url = new URL(joinRequest.url());

	expect(url.searchParams.get('bot_name')).toBe(botName);
	expect(url.searchParams.get('aligner_prompt')).toBe(alignerPrompt);
	expect(url.searchParams.get('bot_prompt')).toBe(botPrompt);
});
